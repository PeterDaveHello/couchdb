...
* Send a valid, empty, EventSource event for heartbeat instead of a newline,
  to avoid clients getting stuck with a half-open TCP state. :commit:`95600b7`
* Add experimental Content-Security-Policy-Support (CSP) for Fauxton to reduce
  risk of XSS attacks. See (:ref:`config/csp`) for more information.
  :commit:`3bcf664`
